...
export function AudiologicalRecordView({ workerId }: AudiologicalRecordViewProps) {
      ...
      try {
        ...
        const formatEarData = (examList: any[], earField: string) => {
          return FREQUENCIES.map(freq => {
            ...
            examList.forEach(ex => {
               point[ex.id] = ex[earField] ? ex[earField][fStr] : null;
            });
            return point;
          });
        };
  ...
  const isCriticalFreq = (freq: number) => [3000, 4000, 6000].includes(freq);
          ...
          <Table verticalSpacing="sm" striped highlightOnHover withTableBorder>
            <Table.Thead className="bg-slate-50">
              <Table.Tr>
                ...
                <Table.Th>Média SS (OD / OE)</Table.Th>
              ...
              {allExams.map((ex, idx) => {
                ...
                return (
                  <Table.Tr key={ex.id}>
                    ...
                    <Table.Td>
                      <Badge variant={isRef ? 'filled' : 'outline'} color={isRef ? 'teal' : 'gray'} size="sm">
                        {ex.exam_type?.toUpperCase()} {isRef && '(BASELINE)'}
                      </Badge>
                    </Table.Td>
                    <Table.Td>
                      <Badge 
        ...
        <Paper p="md" radius="md" withBorder className="bg-slate-50 border-blue-200">
          ...
          <Grid>
            {['OD', 'OE'].map((ear) => {
              ...
              if (!data || data.ref_346 === null) return null;

              const hasPioraIsolada = data.frequencias_isoladas_piora && data.frequencias_isoladas_piora.length > 0;
              const hasPiora346 = data.atual_346 !== null && data.ref_346 !== null && (data.atual_346 - data.ref_346 >= 10);
          ...
          <Paper p="md" radius="md" withBorder className="bg-white">
            <Group justify="space-between" mb="lg">
              ...
              {workerInfo.diagnosisOD && (
                <Group gap="md">
                  <Badge color="red" variant="dot">SS (500,1k,2k): {workerInfo.diagnosisOD.media_oms} dB</Badge>
          ...
          <Paper p="md" radius="md" withBorder className="bg-white">
            <Group justify="space-between" mb="lg">
              ...
              {workerInfo.diagnosisOE && (
                <Group gap="md">
                  <Badge color="blue" variant="dot">SS (500,1k,2k): {workerInfo.diagnosisOE.media_oms} dB</Badge>
