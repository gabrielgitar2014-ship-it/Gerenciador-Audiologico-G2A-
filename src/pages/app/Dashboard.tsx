...
export function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Lógica de Filtro Robusta (Qualquer status diferente de 'normal')
      const alteredCount = monthExams?.filter(e => 
        e.result_status && e.result_status.trim().toLowerCase() !== 'normal'
      ).length || 0;
    ...
    <div className="space-y-8 animate-fade-in pb-10 relative">
      ...
      <Grid gutter="lg">
        ...
        <Grid.Col span={{ base: 12, sm: 6, lg: 3 }}>
          <Paper p="xl" radius="2rem" className="bg-white/40 backdrop-blur-2xl border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300">
            <Group justify="space-between">
              <ThemeIcon size="xl" radius="lg" variant="light" color="blue" className="bg-blue-50/50"><IconUsers size={24} stroke={1.5} /></ThemeIcon>
              <Badge color="gray" variant="dot" className="bg-white/50 border-0 shadow-sm">Ativos</Badge>
            </Group>
            <div className="mt-5">
              <Text size="xs" c="dimmed" tt="uppercase" fw={700} className="tracking-wider">Total de Vidas</Text>
              <Text size="2.5rem" fw={800} className="text-slate-800 leading-tight">{stats.totalLives}</Text>
            </div>
          </Paper>
        </Grid.Col>
        ...
        <Grid.Col span={{ base: 12, sm: 6, lg: 3 }}>
          <Paper p="xl" radius="2rem" className="bg-white/40 backdrop-blur-2xl border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300">
            <Group justify="space-between">
              <ThemeIcon size="xl" radius="lg" variant="light" color="teal" className="bg-teal-50/50"><IconActivity size={24} stroke={1.5} /></ThemeIcon>
              <Tooltip label="Comparativo com o mês anterior">
                <Badge color={stats.examsMonthDiff >= 0 ? 'teal' : 'red'} variant="dot" className="bg-white/50 border-0 shadow-sm" style={{ cursor: 'help' }}>
                  {stats.examsMonthDiff > 0 ? `+${stats.examsMonthDiff}%` : `${stats.examsMonthDiff}%`}
                </Badge>
              </Tooltip>
            </Group>
            <div className="mt-5">
              <Text size="xs" c="dimmed" tt="uppercase" fw={700} className="tracking-wider">Exames (Mês)</Text>
              <Text size="2.5rem" fw={800} className="text-slate-800 leading-tight">{stats.examsMonth}</Text>
            </div>
          </Paper>
        </Grid.Col>
        ...
        <Grid.Col span={{ base: 12, sm: 6, lg: 3 }}>
          <Paper p="xl" radius="2rem" className="bg-white/40 backdrop-blur-2xl border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300">
            <Group justify="space-between">
              <ThemeIcon size="xl" radius="lg" variant="light" color="indigo" className="bg-indigo-50/50"><IconCalendarStats size={24} stroke={1.5} /></ThemeIcon>
              <Badge color="indigo" variant="dot" className="bg-white/50 border-0 shadow-sm">Hoje</Badge>
            </Group>
            <div className="mt-5">
              <Text size="xs" c="dimmed" tt="uppercase" fw={700} className="tracking-wider">Agendados Hoje</Text>
              <Text size="2.5rem" fw={800} className="text-slate-800 leading-tight">{stats.scheduledToday}</Text>
            </div>
          </Paper>
        </Grid.Col>
        ...
        <Grid.Col span={{ base: 12, sm: 6, lg: 3 }}>
          <Paper p="xl" radius="2rem" className="bg-red-50/40 backdrop-blur-2xl border border-red-100/60 shadow-[0_8px_30px_rgba(239,68,68,0.05)] hover:shadow-[0_8px_30px_rgba(239,68,68,0.1)] transition-all duration-300">
            <Group justify="space-between">
              <ThemeIcon size="xl" radius="lg" variant="light" color="red" className="bg-red-100/50"><IconAlertTriangle size={24} stroke={1.5} /></ThemeIcon>
              <Tooltip label="Porcentagem de exames alterados no mês">
                <Badge color="red" variant="filled" className="shadow-sm">
                  {alterationRate}% Taxa
                </Badge>
              </Tooltip>
            </Group>
            <div className="mt-5">
              <Text size="xs" c="red.8" tt="uppercase" fw={700} className="tracking-wider">Exames Alterados</Text>
              <Text size="2.5rem" fw={800} className="text-red-900 leading-tight">{stats.alteredMonth}</Text>
            </div>
          </Paper>
        </Grid.Col>
      </Grid>
      ...
      <Grid gutter="xl">
        ...
        <Grid.Col span={{ base: 12, lg: 8 }}>
          <Paper p="xl" radius="2rem" className="h-full bg-white/40 backdrop-blur-2xl border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
        ...
        <Grid.Col span={{ base: 12, lg: 4 }}>
          <Paper p="xl" radius="2rem" className="h-full bg-white/40 backdrop-blur-2xl border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
            <Title order={4} mb="lg" className="text-slate-800">Últimos Finalizados</Title>
            
            <Stack gap="md">
              {recentExams.length > 0 ? recentExams.map((exam) => (
                <Paper key={exam.id} p="md" radius="xl" className="bg-white/50 border border-white/80 hover:bg-white/80 hover:shadow-sm transition-all duration-300">
              ...
              <Button variant="subtle" fullWidth color="blue" radius="xl" rightSection={<IconArrowUpRight size={14}/>} onClick={() => navigate(`/app/${companyId}/exames`)}>
      ...
      {/* FILA DE ESPERA */}
      <Paper p="xl" radius="2rem" className="bg-white/40 backdrop-blur-2xl border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
        <Group justify="space-between" mb="xl">
          <div>
