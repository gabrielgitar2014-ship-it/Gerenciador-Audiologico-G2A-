...
import { IconBriefcase, IconBuildingFactory, IconEarOff, IconId, IconUser } from '@tabler/icons-react';
...
export function PatientForm({ companyId, onSuccess, onCancel }: PatientFormProps) {
  ...
  const form = useForm({
    initialValues: {
      ...
      gheId: null as string | null,
      expostoRuido: false,
      anosExposicaoRuido: null as number | null,
      usaEpiAuditivo: false,
      ...
      const { error } = await supabase.from('employees').insert([
        {
          ...
          // Exposição ocupacional / EPI / status
          exposto_ruido: values.expostoRuido,
          anos_exposicao_ruido: values.expostoRuido ? values.anosExposicaoRuido : null,
          usa_epi_auditivo: values.usaEpiAuditivo,
  ...
  return (
    <form onSubmit={form.onSubmit(handleSubmit)} className="space-y-4 relative">
      ...
      {/* SEÇÃO 3: EXPOSIÇÃO OCUPACIONAL */}
      <Text size="xs" fw={700} c="dimmed" tt="uppercase">Exposição Ocupacional</Text>

      <Switch
        label="Exposto a Ruído"
        description="Marcar se o trabalhador é exposto a ruído ocupacional"
        thumbIcon={<IconEarOff size={12} />}
        {...form.getInputProps('expostoRuido', { type: 'checkbox' })}
      />

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <NumberInput
          label="Anos de Exposição a Ruído"
          placeholder="Ex: 5"
          min={0}
          max={70}
          disabled={!form.values.expostoRuido}
          {...form.getInputProps('anosExposicaoRuido')}
        />

        <Switch
          label="Usa EPI Auditivo"
          description="Protetor auricular em uso"
          mt={28}
          {...form.getInputProps('usaEpiAuditivo', { type: 'checkbox' })}
        />
      </div>

      <Select
        label="Status do Trabalhador"
        data={[
