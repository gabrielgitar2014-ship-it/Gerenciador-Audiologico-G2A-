import numpy as np
from schemas import Thresholds, ExamType
    ...
    def evaluate(exam_data: dict, exam_type: str, reference_exam: dict = None) -> dict:
        ...
        def analyze_ear_nr07(ear_data, ear_label, ref_ear_data=None):
            ...
            if ref_ear_data:
                ...
                # Ajuste: Apenas considerar frequências preenchidas no exame de referência
                ref_freqs = [v for k, v in ref_ear_data.items() if v is not None and str(k) in ['250', '500', '1000', '2000', '3000', '4000', '6000', '8000']]
                ...
                if ref_is_normal:
                    if piora_media_346 or piora_isolada_346:
                        alerts.append(f"{ear_label}: Desencadeamento de PAINPSE identificado (piora em média tritonal ou frequência isolada agudas).")
                        has_desencadeamento_or_agravamento = True
                else:
                    if piora_media_346 or piora_media_512 or piora_isolada_geral:
                        alerts.append(f"{ear_label}: Agravamento de PAINPSE identificado (piora em média tritonal ou frequência isolada).")
                        has_desencadeamento_or_agravamento = True
